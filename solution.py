    ...
    for name, m in sparsed_model.named_modules():
        if isinstance(m, nn.Conv2d):
            if name in prune_ratio_dict.keys():
                ...
                ##### WRITE CODE HERE #####
                prune.ln_structured(module=m, name='weight', amount=prune_ratio_dict[name], n=2, dim=0)
...
def generate_resnet_layers(model, base_path='common/layer_prob_base.yaml',  path='Q3/prob'):
    ...
    for name, m in model.named_modules():
        if isinstance(m, nn.Conv2d):
            ...
            ##### WRITE CODE HERE #####
            info = {}
            info['in_channels'] = m.in_channels
            info['out_channels'] = m.out_channels - num_zero_channels
            info['kernel_size'] = m.kernel_size[0]
            info['stride'] = m.stride[0]
            info['Height'] = input_activation[name].shape[-2]
            info['Width'] = input_activation[name].shape[-1]
            info['base_path'] = base_path
            info['save_path'] = path+f"/{name}"
            conv_layer_generator(**info)
